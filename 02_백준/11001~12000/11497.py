...
# 우리는 더 나은 배열 [2, 5, 9, 7, 4]를 만들 수 있으며 이 배열의 난이도는 |5-9| = 4이다. 이 배열보다 난이도가 낮은 배열은 만들 수 없으므로 이 배열이 남규가 찾는 답이 된다.
...
for _ in range(case):
    ...
    log.sort()      # 통나무 높이를 오름차순으로 정렬

    difference = 0
    for i in range(len(log)-2):     # 차이를 최소로 하려면 2칸씩 건너뛰면 된다.
        difference = max(difference, log[i+2]-log[i])
    print(difference)
