...
# 입력: 첫째 줄에 멜로디에 포함되어 있는 음의 수 N과 한 줄에 있는 프렛의 수 P가 주어진다. (N ≤ 500,000, 2 ≤ P ≤ 300,000)
# 다음 N개 줄에는 멜로디의 한 음을 나타내는 두 정수가 주어진다. 첫 번째 정수는 줄의 번호이고 두 번째 정수는 그 줄에서 눌러야 하는 프렛의 번호이다.
# 입력으로 주어진 음의 순서대로 기타를 연주해야 한다.
# 출력: 첫째 줄에 멜로디를 연주하는데 필요한 최소 손가락 움직임을 출력한다.
import sys

N, P = map(int, sys.stdin.readline().split())   # 멜로디에 포함되어 있는 음의 수 N, 한 줄에 있는 프렛의 수 P
note = [list(map(int, sys.stdin.readline().split())) for _ in range(N)]
stack = [[] for _ in range(7)]    # 줄마다 프렛을 저장하기 위한 리스트
