...
# 출려기 제시된 부등호 관계를 만족하는 k+1 자리의 최대, 최소 정수를 첫째 줄과 둘째 줄에 각각 출력해야 한다.
# 단 첫 자리가 0인 경우도 정수에 포함되어야 한다. 모든 입력에 답은 항상 존재하며 출력 정수는 하나의 문자열이 되도록 해야 한다.
import sys
from itertools import permutations

k = int(sys.stdin.readline())
inequality_signs = list(map(str, sys.stdin.readline().split()))
numbers = list(range(0, 10))
permute = permutations(numbers, k+1)
flag = False
answer = []

for i in permute:
    for j in range(k):
        if inequality_signs[j] == '>':
            if int(i[j]) > int(i[j+1]):
                flag = True
            else:
                flag = False
                break
        elif inequality_signs[j] == '<':
            if int(i[j]) < int(i[j+1]):
                flag = True
            else:
                flag = False
